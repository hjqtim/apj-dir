...
export default function ChildForm(props) {
  ...
  const Dialog = withStyles(() => ({
    paper: { ...(style && style.childForm ? style.childForm.dialog : {}) }
  }))(HADialog);
  ...
  const useStyles = makeStyles(() => ({
    content: { ...(style && style.childForm ? style.childForm.content : {}) },
    formElementContainer: {
      ...(style && style.childForm ? style.childForm.formElementContainer : {})
    },
    actions: { ...(style && style.childForm ? style.childForm.actions : {}) }
  }));

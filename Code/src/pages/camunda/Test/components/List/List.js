...
function List(props) {
  ...
  const [workFlowName, setWorkFlowName] = useState('Test');
  ...
  useEffect(() => {
    API.getProcessDefinitions({ limit: 20, page: 1 })
      .then(({ data }) => {
        const { list } = data;
        setWorkFlowList([...list]);
        setWorkFlowName(list[0]?.name || 'Test');
      })
      .catch((e) => {
        console.log(e);
      });

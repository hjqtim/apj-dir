...
function List(props) {
  ...
  useEffect(() => {
    Loading.show();
    API.getProcessDefinitions({ limit: rowsPerPage, page: page + 1, type: 'all' })
      .then(({ data }) => {
        setTotal(data.total);
  ...
  const handlePublish = (event, row) => {
    ...
    API.getPublishModel(row.id)
      .then(() => {
        ...
        Loading.hide();
        API.getProcessDefinitions({ limit: rowsPerPage, page: page + 1, type: 'all' }).then(
          ({ data }) => {
            setTotal(data.total);
            handleData(data.list);
          }
  ...
  const customEdit = (e, row) => {
    window.open(
      `${createPrefix}/camunda/platform?id=${row.id}&token=${localStorage.getItem('token')}`
    );
  };
  ...
  const customCreate = () => {
    window.open(`${createPrefix}/camunda/platform?token=${localStorage.getItem('token')}`);

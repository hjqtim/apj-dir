...
const AttachmentControl = (props) => {
  ...
  const filesList = useSelector((state) => state.resourceMX?.fileAttachment);
  // console.log('filesList', filesList);
  const resourceStatus = useSelector((state) => state.resourceMX.resourceStatus); // state 来判断 流程的进度
  const error = useSelector((state) => state.resourceMX?.error?.fileAttachment);
                          ...
                          onClick={() => {
                            fetch(row.fileUrl)
                              ...
                              .then((blob) => {
                                ...
                                a.download = `RM${row.name}`; // 下载文件的名字
                                document.body.appendChild(a);
                                a.click();
                              });
                            // downBySharedDisk(row.fileUrl)
                          }}
                        >

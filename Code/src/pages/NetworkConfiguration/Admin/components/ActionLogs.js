...
const ActionLog = (props) => {
  ...
  const needConvertData = ['available', 'isEnable'];

  const needConvertData2 = ['tempFlag'];
  ...
  const columns = [
    ...
    { field: 'fieldName', headerName: 'Field Name', flex: 1 },
    {
      ...
      renderCell: (params) => {
        ...
        if (needConvertData2.includes(fieldName)) {
          return Number(params?.value) === 1 ? 'Temp' : 'Perm';
        }
        return params?.value || '';
      }
    },
